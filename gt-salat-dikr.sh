...
EOF
        ;;
    '')
        # العرض الافتراضي بدون استخدام local
        zekr_result=$(show_random_zekr)
        [ -n "$zekr_result" ] && echo "$zekr_result"
        if get_next_prayer 2>/dev/null; then
            left_minutes=$((PRAYER_LEFT/60))
            hours_left=$((left_minutes/60))
            minutes_left=$((left_minutes%60))
            printf "\n\e[1;34m🕌 الصلاة القادمة: %s عند %s (باقي %02d:%02d)\e[0m\n" "$PRAYER_NAME" "$PRAYER_TIME" "$hours_left" "$minutes_left"
        fi
        ;;
    *)
        echo "❌ خيار غير معروف: $1"
